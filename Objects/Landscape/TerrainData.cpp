...
void TerrainData::MakeMesh()
{
	...
	for (int z = 0; z < height; ++z)
	{
		for (int x = 0; x < width; ++x)
		{
			...
			vertex.pos.y = pixels[index].x * MAX_HEIGHT; // 픽셀 위치에서 가죠온 R(레드)값
...
void TerrainData::AlignVertexData()
{
	...
	//각 정점의 인접 관계에 따라 다른 목록으로 한번 더 만들기
	...
	for (UINT z = 0; z < height - 1; ++z) // 위치에 따른 정점과 인접 정보 얻기
	{
		for (UINT x = 0; x < width - 1; ++x)
		{
			...
			UINT index2 = width * (z + 1) + (x + 0);//세로기준 옆자리
