...
void ModelExporter::ReadMaterial()
{
    for (UINT i = 0; i < scene->mNumMaterials; i++)
    {
        ...
        wstring aa = ToWString(CreateTexture(file.C_Str()));
        material->SetDiffuseMap(aa);
        file.Clear();
...
string ModelExporter::CreateTexture(string file)
{
    ...
    string fileName2 = file.c_str();
    while (fileName2.find("\\") != string::npos)
    {
        fileName2 = fileName2.replace(fileName2.find("\\"), 1, "/");
    }

    const aiTexture* texture = scene->GetEmbeddedTexture(fileName2.c_str());
