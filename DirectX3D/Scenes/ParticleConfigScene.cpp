...
ParticleConfigScene::ParticleConfigScene()
{
	...
	wstring  FILENAME = L"Textures/Effect/flower.png";
	...
	instanceBuffer = new VertexBuffer(instances.data(), sizeof(InstanceData), particleCount);
...
void ParticleConfigScene::GUIRender()
{
	...
	ImGui::SliderFloat3("MinVelocity", (float*)&data.minVelocity, -100, 100);
	ImGui::SliderFloat3("MaxVelocity", (float*)&data.maxVelocity, -100, 100);
	ImGui::SliderFloat3("MinAcceleration", (float*)&data.minAcceleration, -100, 100);
	ImGui::SliderFloat3("MaxAcceleration", (float*)&data.maxAcceleration, -100, 100);
	ImGui::SliderFloat3("MinStartScale", (float*)&data.minStartScale, 0, 300);
	ImGui::SliderFloat3("MaxStartScale", (float*)&data.maxStartScale, 0, 300);
	ImGui::SliderFloat3("MinEndScale", (float*)&data.minEndScale, 0, 300);
	ImGui::SliderFloat3("MaxEndScale", (float*)&data.maxEndScale, 0, 300);
	...
	ImGui::SliderFloat("MaxSpeed", (float*)&data.maxSpeed, data.minSpeed, 1000);
	ImGui::SliderFloat("MinStartTime", (float*)&data.minStartTime, 0, data.maxStartTime);
	ImGui::SliderFloat("MaxStartTime", (float*)&data.maxStartTime, data.minStartTime, data.duration); // 아무리 지연되도 파티클이 있어야 할 시간 내에는 나오게

	//여기까지 수행되면 디버그 패널에 설정 옵션이 나오면서 설정에 의한 조작 가능
	SaveDialog(); // 세이브
	ImGui::SameLine(); // 바로 위 UI와 바로 아래 UI가 같은 줄에 나오게 하는 기능
...
void ParticleConfigScene::UpdatePhysical()
{
	...
	FOR(data.count) 
	{
		...
		particleInfo[i].transform.Scale() = Lerp(particleInfo[i].startScale, particleInfo[i].endScale, t);

		particleInfo[i].transform.UpdateWorld();		
		instances[drawCount++].transform = XMMatrixTranspose(particleInfo[i].transform.GetWorld());
...
void ParticleConfigScene::Save(string file)
{
	...
	//파티클에 가장 필요한 것 : 어떤 그림의 파티클인가 ( 생성때도 이미지를 썼으니까)
...
void ParticleConfigScene::Load(string file)
{
	...
	quad->GetMaterial()->SetDiffuseMap(textureFile);// 그 그림파일의 대상을 쿼드 디퓨즈맵에 할당
...
void ParticleConfigScene::EditTexture()
{
	...
	ImTextureID textureID = quad->GetMaterial()->GetDiffuseMap()->GetSRV(); //이미지 데이터 처리후 셰이더에 넘기는 과정의 데이터

	if (ImGui::ImageButton(textureID, ImVec2(50, 50)))
		//받아온 이미지 데이터를 imgui 방식으로, 50*50 크기로 보여주는 버튼을 그려주고, 이 버튼이 눌리면
	{
