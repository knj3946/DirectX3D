...
void Spark::GUIRender()
{
    ...
    ImGui::SliderFloat("MinRadius", &minRadius, 1.0f, maxRadius);//최소 반경 - 최대값이 최대 반경보다는 작아야됨
    ImGui::SliderFloat("MaxRadius", &maxRadius, minRadius, 50.0f);//최대 반경 - 최소값이 최소반경보다는 커야됨
...
void Spark::Play(Vector3 pos)
{
    ...
    Particle::Play(pos); // 매개변수를 전달해서 부모함수 실행
...
void Spark::UpdateParticle()
{
    ...
    FOR(particleCount)
    {
        ...
        Float3 rot; //방향계산
        rot.x = Random(0.0f, XM_2PI); // XM_2PI : 호도로 계산된, "반경*원주율"의 2배 = 360도
        ...
        //위에서 나온 반경-방향-속력 기준으로 "속도"를 만들기 (=방향과 힘을 모두 가진 벡터)

        Matrix matRot = XMMatrixRotationRollPitchYaw(rot.x, rot.y, rot.z);//방향과 크기를 가진 공간부터 만들기
                                //XMMatrix~~ : 벡터를 크기를 갖춘 공간으로 만드는 DX제공 함수
        ...
        XMStoreFloat3(&vertices[i].velocity, XMVector3TransformCoord(velocity, matRot));
                                ...
                                //->XMVector3TransformCoord 실행의 결과를 &vertices[i].velocity 데이터에 담음

        //위 과정으로 각 정점이 다음 순간에 (혹은 다음 틱에) 가야할 곳을 지정
