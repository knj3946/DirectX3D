...
private:
	...
	struct ParticleData //파티클 자체의 데이터 = 전체의 옵션
	{
		...
		UINT count = 0; //파티클의 허용 개수 혹은 파티클 내 입자 인스턴스의 개수 (기본은 후자)
		...
		Vector3 minStartScale = { 1,1,1 }; // 파티클 생성시 최소 크기
		Vector3 maxStartScale = { 1,1,1 }; // 파티클 생성시 최대 크기
		Vector3 minEndScale = { 1,1,1 }; // 파티클 종료시 최소 크기
		Vector3 maxEndScale = { 1,1,1 }; // 파티클 종료시 최대 크기
		...
		Float4 startColor = { 1,1,1,1 }; // 시작시 출력할 색상 범위(혹은 색상 보정)
		Float4 endColor = { 1,1,1,1 }; // 종료시 출력할 색상 범위(혹은 색상 보정)

		//-> 물리현상, 셰이더 연산 등을 고려하지 않고 의도 가능한 최소한의 옵션을 작성
	...
	struct ParticleInfo
	{
		Transform transform; // 각 입자 혹은 전체 중심의 트랜스 폼
		Vector3 velocity; //위 데이터에서 랜덤으로 나온 속력기준 (=방향, 기준속도)
...
private:
	...
	void LoadData(string file); // C++에 의한 파일 읽기 : 텍스트 읽기 클래스로 사전 설정된 파일 읽기 (있다고 치고)
	void LoadData(wstring file); // 윈도우에 의함 파일 읽기 
								 // : 이미지 리소스를 직접 읽기
...
private:
	...
	//이미지 출력용....
	BlendState* blendState[2];//이미지 혼합용(다른 렌더 이미지와 범용하기 위함)
