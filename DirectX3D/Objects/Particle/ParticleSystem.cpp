...
void ParticleSystem::Render()
{
	...
	DC->DrawIndexedInstanced(6, drawCount, 0, 0, 0);//마지막 매개 변수 3개는 다른 곳에서 설정을 해두고 여기선 0,0,0 하면 가장 속이 편하다
	//DrawIndexedInstanced:준비된 하나의 출력 데이터를 렌더 절차가 받은 인덱스에 따라
							//반복해서 출력하는 기능을 담당
							//매개변수 : 한 번에 그릴 정점 = 출력할 인스턴스의 단위(현재 : 한번에 정점 6개씩)
							// ,출력할 횟수(드로우 콜 횟수만큼)
							// ,어디부터,어디로(기준값 없음 = 정해진 설정이 있으면 설정)
							// ,추가 참조할 자료가 있다면 어디에 있는가 (=지금은 없음)	
...
void ParticleSystem::UpdatePhysical()
{
	...
	FOR(data.count) // 각 입자에 코드 반영
	{
		...
		//아직 생애주기가 시작할 때도차 되지 않은 것 =시작할 때가 안된 것
		...
		//* 왜 카메라인가 (벡터가 아니고) -> 벡터를 쓰면 카메라와 강향을 맞추기 보다 // 이미지가 카메라'를' 바라보게 된다->보는 입장에서 꼭 정면 100%라는 보장이 없어서
		...
		instances[drawCount++].transform = XMMatrixTranspose(particleInfo[i].transform.GetWorld()); //이 호출이 끝나면 드로우 콜 횟수에 +1
												//공간을 나타내는 행렬 데이터를 전이
...
{
	float t = lifeSpan / data.duration; // 진행을 하고 말고, 시작 시간을 포함을 하고 말고는 위쪽 함수에서 끝나서, 여기선 생략
...
void ParticleSystem::Init()
{
	...
	else blendState[1]->Alpha(true); // 배경색 투명화 (중간에 옵션이 바뀔 수도 잇어서 여기서 확인사살)
	...
	//data.count particleCount가 달랐다면 여기서 초기화
	...
	for (ParticleInfo& info : particleInfo) // 개별 인스턴스 내 정보들을 호출해서 수정
	{
		...
		//옵션  : 속력(빠르기가 있는 벡터)을 기준속도(길이1 + 방향)로 수정한다면 아래 코드까지
...
void ParticleSystem::LoadData(wstring file)
{
	...
	//만든 쿼드에 메티리얼을 세팅 (그림을 바로 안 쓰는 이유 : 바로 쓰면 쿼드 크기가 그림을 따라감)
	quad->GetMaterial()->SetDiffuseMap(file);
	quad->GetMaterial()->SetShader(L"Effect/Particle.hlsl"); // 기본 파티클 셰이더 (더 기본으로 가려면 텍스처 셰이더)
