...
private:
	float MAX_HEIGHT = 20;
	UINT MIN_TRIANGLE = 2048; // 자료 구조를 나눌 때, 각 단위가 성립하기 위한 최소 감각형 개수
				...
				//....A*에 쓰이는 것과는 다르다
	{
		...
		Node* children[4] = {}; // 트리 구조 = 부모 밑에 동일 한 타입의 자식들이 있음
								// 자식이 넷이라서 쿼드 트리
	};
public:
...
private:
	...
	//메쉬의 위치 계산
	void CalcMeshDimension(UINT vertexCount, float& centerX, float& centerZ, float& size);
