...
QuadTreeTerrain::QuadTreeTerrain(wstring heightFile)
{
    ...
    vertices = terrainData->GetVertices(); //지형 데이터에서 정리된 정점 받기
...
void QuadTreeTerrain::Render()
{
    ...
    drawCount = 0; // 드로우 콜 횟수 초기화 (계산은 렌더 노드 중에 할것)
    SetRender(); // 게임 오브젝트로서
...
void QuadTreeTerrain::RenderNode(Node* node)
{
    ...
    //절두체 적용
    if (!CAM->ContainPoint(center, radius)) // 중심과 반경까지
    {
        //절두체 안에 없으면
        return;//렌더 안함
    }
    ...
    if (count != 0)
    {
        ...
        // 반대로 부모 노드를 독립수행 시킬분은 여기서 코드를 추가 작성
    }

    //여기까지 오면 자식 노드라고 가정하고, 해당 자식 노드로서의 렌더 수행

    node->mesh->Draw(); // 매쉬를 출력
    drawCount += node->triangleCount; // 노드가 갖고 있었던 삼각형 개수를 드로우콜 합산에 추가(테스트용)
...
void QuadTreeTerrain::DeleteNode(Node* node)
{
    ...
    //먼저 자식이 잇는 경우 자식부터 하나하나 추적하면서 삭제 내용을 선행

    //더 자식이 없으면 자기 노드의 내부 데이터 삭제
    delete node->mesh; // 노드 구조체 안에 있었던 매시 삭제

    // * 지금은 소멸자용이지만 나중에 노드에 수동 편집이 이루어질 수 있다면
    // 이 함수를 다른곳에서도 호출해야 할 수 있다 (노드 삭제니까)
}
...
void QuadTreeTerrain::CalcMeshDimension(UINT vertexCount, float& centerX, float& centerZ, float& size)
{
    ...
    FOR(vertexCount)
    {
        ...
        float depth = abs(vertices[i].pos.z - centerZ); // 결과 확인 + 정점 실제 위치에서 
                                                        // 가로 세로를 다시 도출

        if (width > maxX) maxX = width; // 큰쪽을 "max"값에 계속 갱신
    ...
    // 3. 2의 결과로 최대 범위 구함
    // 4. 3의 결과에서 가로 세로의 크기 범위까지 예측 가능

    //그래서 나온 4의 결과를 size에 저장

    size = max(maxX, maxZ) * 2; // 현재 지형의 최대 크기 범위(더 큰쪽 변이 얼마나 긴가?)
}
...
bool QuadTreeTerrain::IsTriangleContained(UINT index, float x, float z, float size)
{
    ...
    if (minX > (x + halfSize)) return false; //가장 왼쪽에 있어야 할 점이 가장 오른쪽보다 오른쪽이면 -> 삼각형은 여기 없음
    ...
    //여기까지 오면 영역에 삼각형을 구성하는 점이 하나도 없는 상황 제외한것
    //일단 삼각형의 각 점 중 하나는 영역 내에 있는것
    //그다음, 부분 포함 여부를 살펴볼 수 있다
    ...
    if(!countBoundary){
        ...
        float maxX = max(x1, max(x2, x3));
        if (maxX > (x + halfSize)) return false; // 가장 큰 값이 영역 밖이다 = 점하나는 영역 밖이다
    ...
    //삼각형을 이루는 세점이 영역에 들어가는지 여부의 판단 기준에 완전히 부합한 것
    // - >false가 한번도 뜨지 않았다

    return true;

    //따라서...
    //경계면을 따진다면 : 적어도 하나 이상의 점은 영역안에 있다
    //경계면을 제외하면 : 모든 점은 영역안에 있다
}
...
UINT QuadTreeTerrain::ContainedTriangleCount(float x, float z, float size)
{
    ...
    // 방법 1 : 지형 데이터 안에 상세한 삼각형 좌표 및 인덱스 정보를 기록해 뒀다가, 나뉜 규격에 따라
    // 방법 2 : 무식하게 모든 삼각형을 직접 돌려보면서 조건에 맞는지 안 맞는지 본다. 맞으면 +1

    //-> 어쨌든 정점도, 지형 데이터도, 하나하나 직접 만들어 쓰고 있는 지금은 방법 1은 힘드니까
    // 방법 2를 통해서 삼각형 개수를 영역에서 구한다(->헤더가 더 상세해지는 환경이 있다면 거기서
...
void QuadTreeTerrain::CreateTreeNode(Node* node, float x, float z, float size)
{
    ...
    //해당 조건을 표현한 조건문
    if (triangles > MIN_TRIANGLE) // 만약 트리 구분의 조건이 달라지면 여기서 수행
        //예 : 높이가 급격하게 달라진다든다....
    {
        FOR(4) // 자식 트리를 위한 분할 반복
        {
            ...
            //보정치 계싼 후 , 새로운 노드를 배열 내 각 자식 자리에 할당해주고
    ...
    FOR(triangleCount)
    {
        if (IsTriangleContained(i, x, z, size)) // 반복문 속 인덱스 i로 지정된 삼각형이 영역에 있는가
        {
            ...
            vertexIndex = i * 3 + 0;
            vertices[index] = this->vertices[vertexIndex]; // 터레인 데이터에 있었던 정리된 정점 데이터로부터 정보를 얻어와 현재 노드내 벡터의 index 순번에 집어 넣기
            index++;//인덱스+1(뒤로 진행)
            ...
            //이 반복문을 삼각형 개수만큼 돌려주면 각 정점의 정보를 따로 쪼개어서 각 노드에 나눠 넣을 수 있게 될 것
